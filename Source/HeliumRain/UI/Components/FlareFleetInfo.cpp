...
FText SFlareFleetInfo::GetTravelText() const
{
	if (TargetSectorTravel)
	{
		...
		{
			int64 TravelDuration = UFlareTravel::ComputeTravelDuration(PC->GetGame()->GetGameWorld(), TargetFleet->GetCurrentSector(), TargetSectorTravel, PC->GetCompany(),TargetFleet);
...
FText SFlareFleetInfo::GetRepairText() const
{
	...
	return TargetFleet->GetRepairText();
}
...
FText SFlareFleetInfo::GetRefillText() const
{
	...
	return TargetFleet->GetRefillText();
}
